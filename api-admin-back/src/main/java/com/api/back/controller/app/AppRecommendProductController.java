...
@Api(tags = "产品推荐")
@RestController
@RequestMapping("/appadmin/main/recommendProduct")
public class AppRecommendProductController {

    @SofaReference
    private AppRecommendService recommendService;
    ...
    public Response list(PageQuery pageQuery){
        return Response.success(recommendService.list(pageQuery));
    }
    @PostMapping("add")
    @ApiOperation("新增推荐")
    @ApiImplicitParams({
    ...
    public Response disable(Integer id, Boolean disable){
        recommendService.disable(id,disable?0:1);
        return Response.success(true);
    }
}
